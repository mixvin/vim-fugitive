...
if !exists('g:fugitive_status_log_format')
  let g:fugitive_status_log_format = '%h%x09%s'
endif

function! s:QueryLog(refspec, limit, dir) abort
  let [log, exec_error] = s:LinesError(['log', '-n', '' . a:limit, '--pretty=format:' . g:fugitive_status_log_format] + a:refspec + ['--'], a:dir)
